      ...
      -o option-name
          Set the variable corresponding to option-name:
              ...
              pipefail     the return value of a pipeline is the status of
                           the last command to exit with a non-zero status, or zero if no command exited with a non-zero status
...
```shell
## 经常使用的几个attribute, 有帮助于测试
set -x:  debug the shell execute
set -o pipefail:  exit execute if pipeline return value is not zero
set -e: exit immediately if any cmd return non-zero value
